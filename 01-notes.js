...
let bool = Boolean("true"); // This will give us true because the string "true" is a truthy value. But if we do Boolean("false"); it will also give us true because the string "false" is also a truthy value. In JavaScript, all non-empty strings are considered truthy values, regardless of their content. Only an empty string ("") is considered a falsy value. So, Boolean("") will give us false.
...
// Arrow function cannot be used as a parameter because they do not have their own 'this' context. They inherit 'this' from the parent scope. This can lead to unexpected behavior when using arrow functions as callbacks or event handlers. For example, if we have an object with a method that uses an arrow function as a callback, the 'this' keyword inside the arrow function will refer to the parent scope, not the object itself. This can cause issues when trying to access properties or methods of the object. It is recommended to use regular functions instead of arrow functions when we need to use 'this' in a callback or event handler.